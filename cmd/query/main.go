...
func main() {
	...
	if cfg.DisableAuth {
		...
		v1.Use(auth.GinMiddleware(v, auth.ScopeQuery))
	}
	queryapi.Register(v1, cfg, corpus)

	log.Printf("query listening on %s", cfg.HTTPAddr)
